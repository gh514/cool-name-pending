...
let channel = open_out "translated.smt"

let out = Printf.fprintf channel
...
let rec init_grid r c m =
  ...
  else let outputs = Printf.fprintf channel "\n(declare-const r%ic%i Int)" r c; in 
...
and translate_expr e = 
  (match e with
    ...
    | _ ->  out " ");
  match e with 
    | Ast.Integer(i)-> Printf.fprintf channel "%d" i
    | Ast.Boolean(b) -> Printf.fprintf channel "%b" b
    ...
    | Ast.Var(v) -> Printf.fprintf channel "%s" v
