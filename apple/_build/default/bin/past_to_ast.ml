...
let line_constraints l s x line = 
  ...
  in List.map (fun rc -> Ast.Dec(Ast.Bool, to_cell rc "")) cells
    ...
    @ (if line then [] else List.map (fun rc1 -> Ast.Op(to_cell rc1 "_sink", Ast.LeftImp, Ast.UnaryOp(Ast.Not, Ast.MultiOp(Ast.Or, 
      List.map (fun rc2 -> to_var rc1 rc2) (surrounding x rc1))))) cells)
    @ (if line then [] else List.map (fun rc1 -> Ast.Op(Ast.Op(Ast.UnaryOp(Ast.Not, to_cell rc1 "_source"), Ast.And, to_cell rc1 ""), Ast.LeftImp, 
      Ast.Op(Ast.MultiOp(Ast.Add, List.map (fun rc2 -> Ast.ITE(to_var rc2 rc1, Ast.Integer(1), Ast.Integer(0))) (surrounding x rc1)), Ast.GTE, Ast.Integer(1)))) cells)
...
and translate_spec_term l e1 sop e2 vars =
  ...
  match sop1 with
    ...
    | Past.DiagAdjacent -> let Past.RC(_, Past.Integer(_, r1), Past.Integer(_, c1)) = e1 in 
      let Past.RC(_, Past.Integer(_, r2), Past.Integer(_, c2)) = e2 in
      ((if (abs (r1-r2) <= 1 && abs(c1-c2) <= 1 && (r1, c1) <> (r2, c2)) then Ast.Boolean(true) else Ast.Boolean(false)), vars2)
...
and translate_member e1 e2 vars = 
  let base = (let (Ast.Var(v2), vars2) = translate_expr e2 vars
    in match e1 with
      ...
      | _ -> 
        let (Ast.Var(v1), _) = translate_expr e1 vars in (Ast.Var(sprintf "%s_%s" v2 v1), vars2))
        in match e2 with
          | Past.Var(_, v2) -> (match safe_find e2 vars with
            | (dt, Some (Past.List(_, box))) -> (if dt = Past.Box then 
            ...
            | (dt, None) -> let Past.List(_, cells) = e1 in (Ast.MultiOp(Ast.And, List.map 
              (fun rc -> let (r, c) = get_rc rc vars in Ast.Var(sprintf "%s_r%ic%i" v2 r c)) cells), vars))
    | _ -> base
